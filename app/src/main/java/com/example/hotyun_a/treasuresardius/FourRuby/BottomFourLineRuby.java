...
public class BottomFourLineRuby {
    public long scores;
    VertexBufferObjectManager vObject;
    int col,row;
    private int isSprite;

    public BottomFourLineRuby(long score, int col, int row, VertexBufferObjectManager vb){
        this.scores=score;
        this.vObject=vb;
        this.col=col;
        this.row=row;
    }
    public boolean checkLineFourRubyBottom(Text bitmapText,Rectangle masRect[][]){
        if(col!=(MainActivity.countRectHeight-1)&&col!=(MainActivity.countRectHeight-2)&&
                row!=0&&row!=1&&row!=(MainActivity.countRectWidth-1)){
        ...
        if(col!=(MainActivity.countRectHeight-1)&&col!=(MainActivity.countRectHeight-2)&&row!=0&&
