...
public class TopFiveLineRuby {

    public long scores;
    VertexBufferObjectManager vObject;
    int col,row;

    public TopFiveLineRuby(long score, int col, int row, VertexBufferObjectManager vb){
        this.scores=score;
        this.vObject=vb;
        this.col=col;
        this.row=row;
    }
    public boolean checkLineFiveRubyTop(Text bitmapText,Rectangle masRect[][]){
        if(row!=0&&row!=1&&row!=(MainActivity.countRectWidth-1)&&row!=(MainActivity.countRectWidth-2)&& col!=0&&col!=1) {
